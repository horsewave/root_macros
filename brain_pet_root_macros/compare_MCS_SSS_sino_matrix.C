 ...
 ** Function: compare the defference of the scatter sinograms aquired by SSS and MCS;
   ...
   * 
     ...
     * 19: void save_sino_matrix_to_root(TMatrix *mat, int row=256, int column=192)
...
void compare_MCS_SSS_sino_matrix()
//void compare_MCS_SSS_sino_matrix(int view_index)
{
  ...
  string part_name_mcs="scatterMCS/run_num_"+ run_1+"/XB1BN310N-BI-01_0-1800_gpuSimu_scattersino_scaled.fs";
  string part_name_sss="scatterMCS/run_num_"+ run_2+ "/XB1BN310N-BI-01_0-1800_gpuSimu_scattersino_scaled.fs";

  
  string part_root_file_name="root_file/compare_MCS_SSS_scatter_sino_plane_dif_runs_"+run_1+"-"+ run_2+".root";
...
/////////////running without input parameters//////////////////////////////////////////////////
  ...
  int dimx =256;
  int dimy =192;
  TMatrix* mat_mcs= new TMatrix(dimx,dimy);
  TMatrix* mat_sss= new TMatrix(dimx,dimy);


   float_vec_t vector_events_radial_mcs=Get_proj_per_view_sino(path_sino_mcs,mat_mcs,view_index);
   
   float_vec_t vector_events_radial_sss=Get_proj_per_view_sino(path_sino_sss,mat_sss,view_index);

   //draw the sino matrix
   //draw the sino matrix
   string can_name_mcs="canvas_sino_"+ run_1;
   string can_name_sss="canvas_sino_"+run_2;

  save_sino_matrix_to_root(mat_mcs,can_name_mcs,saved_path_root_file);
  save_sino_matrix_to_root(mat_sss,can_name_sss,saved_path_root_file);
...
for(int i=0;i<=vector_events_radial_mcs.size();i++)
 ...
 //write_vector_to_text(saved_path_data, vector_events_radial_mcs); 
  ...
  int proj_num=vector_events_radial_mcs.size();
  float* array_events_per_z_mcs= new float[proj_num];
  float* array_events_per_z_sss= new float[proj_num];
  //float* array_events_differ= new float[proj_num];
  memset(array_events_per_z_mcs,0,sizeof(float)*proj_num);
  memset(array_events_per_z_sss,0,sizeof(float)*proj_num);
  //memset(array_events_differ,0,sizeof(float)*proj_num);
...
void Generate_save_mcs_sss_sino_plot(float*array_events_per_z_mcs, float* array_events_per_z_sss,int array_size,string saved_data_path)
{
  ...
  string canvas_name="canvas_scat_sino_proj_per_view";
...
float_vec_t Get_proj_per_view_sino(string path_sino_input, TMatrix* mat,int view_index)
{
...
void save_sino_matrix_to_root(TMatrix *mat,string canvasTitle,string saved_root_path, int row=256, int column=192)
{

  //string canvasTitle="canvas_MCS_vs_SSS_img";

  //TCanvas* can_hist=new TCanvas(canvasTitle.c_str(),canvasTitle.c_str(), 1000, 800);
  TCanvas* can_hist=new TCanvas(canvasTitle.c_str(),canvasTitle.c_str(), 0,66,1505,984);
can_hist->Range(-143.2065,-24.25806,300.5859,216);
   can_hist->SetFillColor(10);
   can_hist->SetBorderMode(0);
   can_hist->SetBorderSize(2);
   can_hist->SetLeftMargin(0.322688);
   can_hist->SetFrameFillColor(0);
   can_hist->SetFrameLineColor(10);
   can_hist->SetFrameBorderMode(0);
   can_hist->SetFrameFillColor(0);
   can_hist->SetFrameLineColor(10);
   can_hist->SetFrameBorderMode(0);
  ...
  TPaletteAxis *palette = new TPaletteAxis(239.4648,1.903503e-07,254.8189,192,h2_sino);
...
palette->SetTitleSize(0.04);
   ...
   h2_sino->GetListOfFunctions()->Add(palette,"br");
   h2_sino->SetLineColor(10);
   h2_sino->SetLineWidth(0);
   h2_sino->GetXaxis()->SetTitle("View");
   h2_sino->GetXaxis()->CenterTitle(true);
   h2_sino->GetXaxis()->SetNdivisions(508);
   h2_sino->GetXaxis()->SetLabelFont(22);
   h2_sino->GetXaxis()->SetLabelOffset(0.01);
   h2_sino->GetXaxis()->SetLabelSize(0.05);
   h2_sino->GetXaxis()->SetTitleSize(0.05);
   h2_sino->GetXaxis()->SetTitleOffset(0.9);
   h2_sino->GetXaxis()->SetTitleFont(22);
   h2_sino->GetYaxis()->SetTitle("Projection Angle");
   h2_sino->GetYaxis()->CenterTitle(true);
   h2_sino->GetYaxis()->SetNdivisions(508);
   h2_sino->GetYaxis()->SetLabelFont(22);
   h2_sino->GetYaxis()->SetLabelOffset(0.008);
   h2_sino->GetYaxis()->SetLabelSize(0.05);
   h2_sino->GetYaxis()->SetTitleSize(0.05);
   h2_sino->GetYaxis()->SetTitleOffset(0.9);
   h2_sino->GetYaxis()->SetTitleFont(22);
   h2_sino->GetZaxis()->SetLabelFont(42);
   h2_sino->GetZaxis()->SetLabelSize(0.035);
   h2_sino->GetZaxis()->SetTitleFont(42);
   h2_sino->Draw("colz");
   
   TPaveText *pt = new TPaveText(0.4023246,0.9345349,0.5976754,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(0);
   pt->SetFillColor(0);
   pt->SetFillStyle(0);
   ...
   pt = new TPaveText(226.6139,195.6348,270.7459,208.0086,"br");
   pt->SetBorderSize(0);
   pt->SetFillColor(0);
   TText *text = pt->AddText("counts");
   pt->Draw();
   can_hist->Modified();
   can_hist->cd();
   can_hist->SetSelected(can_hist);
   can_hist->ToggleToolBar();
...
save_canvas_to_root_file(can_hist, saved_root_path);
