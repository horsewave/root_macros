...
float_vec_t Get_axile_events_sino_seg_0(string sino_data_path,bool is_float)
{
 ...
 array_events_per_z= new float[dimz];
  ...
  float scaling_factor[153]={0};

  for(int j=0;j<dimz;j++)
  {
    if(j==0||j==(dimz-1))
    {
      scaling_factor[j]=5.0;
    }
    else if(j==1||j==(dimz-2))
    {
      scaling_factor[j]=5.0/2.0;
    }
    else if(j==2||j==(dimz-3))
    {
      scaling_factor[j]=5.0/3.0;
    }
    else if(j%2==1)
    {
       scaling_factor[j]=5.0/4.0;

    }
     else
    {
       scaling_factor[j]=5.0/5.0;
...
for(int j=0;j<dimz;j++)
  {
     array_events_per_z[j]=array_events_per_z[j]* scaling_factor[j];
