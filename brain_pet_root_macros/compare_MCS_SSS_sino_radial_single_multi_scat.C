...
const int mcs_num=3;
void compare_MCS_SSS_sino_radial_single_multi_scat()
{
...
/////////////running without input parameters//////////////////////////////////////////////////
  ...
  string part_name_mcs[mcs_num]={"scatterMCS/scatter_scaled_normed_multi_plus_single_bad_plane_cor.fs",
                                  "scatterMCS/scatter_scaled_normed_single_bad_plane_cor.fs",
                                  "scatterMCS/scatter_multi_bad_plane_cor_scaled.fs"
  };
 
  /*string part_name_mcs[mcs_num]={"scatterMCS/scatter_multi_plus_single.fs",*/
                                  //"scatterMCS/scatter_single.fs",
                                  //"scatterMCS/scatter_multi.fs"
  /*};*/


  string part_name_sss="scatterSSS/scater_bad_plane_cor.fs";
  //string part_root_file_name="root_file/compare_MCS_SSS_scatter_single_multi.root";
...
for(int i=0;i<mcs_num;i++)
  {
    ...
    vector_events_radial_mcs[i] = Get_radial_events_sino(path_sino_mcs[i]);
    cout<< "finish read file: "<< i<<endl;

  }

    cout<< "read sss sino: "<<endl;
  float_vec_t vector_events_radial_sss=Get_radial_events_sino(path_sino_sss);
    cout<< "finish sss sino: "<<endl;
...
for(int i=0;i<vector_size;i++)
  {
    if(vector_events_radial_mcs[0][i]==0)
    {
      vector_events_radial_mcs[0][i]=vector_events_radial_mcs[2][i]*2;
...
for(int i=0;i<vector_size;i++)
  {
    if(vector_events_radial_mcs[1][i]==0)
    {
      vector_events_radial_mcs[1][i]=vector_events_radial_mcs[2][i]*1;
...
/*}*/
  string saved_vector_name_mcs[mcs_num]={"vector_events_radial_mcs_total",
                                         "vector_events_radial_mcs_single",
                                         "vector_events_radial_mcs_multi"
                                         //"vector_events_radial_mcs_4",
  };


  string saved_vector_name_sss="vector_events_radial_sss";
  cout<< " write vector to root"<<endl;
  for(int i=0;i<mcs_num;i++)
{
 ...
 //write_vector_to_text(saved_path_data, vector_events_radial_mcs); 
  ...
  cout<<"the vector size is:"<<vector_events_radial_mcs[0].size()<<endl;
  ...
  int view_num=vector_events_radial_mcs[0].size();
  float* array_events_per_z_mcs[mcs_num];
 
  for(int i=0;i<mcs_num;i++)
{
  array_events_per_z_mcs[i]= new float[view_num];
  memset(array_events_per_z_mcs[i],0,sizeof(float)*view_num);
}
...
void Generate_save_mcs_sss_sino_plot(float*array_events_per_z_mcs[mcs_num], float* array_events_per_z_sss,int array_size,string saved_data_path)
{
  ...
  string canvas_name="canvas_scat_sino_radial_multi_run";
 ...
 cout<<"step 2 finishes"<<endl;
  ...
  Color_t mcs_color[mcs_num]={4,1,3};
  /*for(int i=0;i<mcs_num;i++)*/
  //{
    //mcs_color[i]=i+1;
  /*}*/
...
for(int i=0;i<mcs_num;i++)
 ...
 cout<<"step 6 finishes"<<endl;
  ...
  mcs_text[i] = "GGEMS counts: "+string_total_counts_mcs[i];
  }
...
float_vec_t Get_radial_events_sino(string path_sino_input)
{
 ...
 string path_sino_gaps = "/data/PET/mr_pet_temp/Ma/software/data/gpupet/phantom/cylinder_phantom_norm_file_mcs/common_sino_files/gaps_sino.fs" ;
  ...
  //get_sino_radial_pro_seg_0( sino_data, radial_sino);
...
void get_sino_radial_pro_seg_0(float* sino_data, double* radial_sino,int dimx=256, int dimy=192, int dimz=153)
{
   
  int dimz_start=70;
  int dimz_end=80;
  //for(int z=0;z<dimz;z++)
  for(int z=dimz_start;z<dimz_end;z++)
  {
    int temp_z_index=z*dimy*dimx;
    for (int y=0;y<dimy;y++)
    {
