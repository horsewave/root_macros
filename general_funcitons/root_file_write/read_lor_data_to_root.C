...
{  
  ...
  string graph_title="my_graph";

Save_1d_graph_to_root( result_path,sum_ring_events, ring_num, graph_name,graph_title);
...
void write_array_to_tree(string path_saved_root_file, float *array_data,int array_size)
{
...
void write_array_to_root(string path_saved_root_file, float *array_data,int array_size)
{
  ...
  //4: write to the file.
  v->Write("lor_events_axile_distribution",TObject::kOverwrite);//// save only the new version of the tree
  f->Close();
